...
def load_dataset() -> [CUDAPrefetcher, CUDAPrefetcher]:
    ...
    train_datasets = PairedImageDataset(srresnet_config.train_gt_images_dir, srresnet_config.train_lr_images_dir)
    test_datasets = PairedImageDataset(srresnet_config.test_gt_images_dir, srresnet_config.test_lr_images_dir)
...
def train(
    ...
    while batch_data is not None:
        ...
        loss_weights = torch.Tensor(srresnet_config.loss_weights).to(srresnet_config.device)

        # Image augmentation operations
        gt, lr = imgproc.random_crop_torch(gt, lr, srresnet_config.gt_image_size, srresnet_config.upscale_factor)
        gt, lr = imgproc.random_rotate_torch(gt, lr, srresnet_config.upscale_factor, [0, 90, 180, 270])
        ...
        with amp.autocast():
            ...
            loss = torch.sum(torch.mul(loss_weights, criterion(sr, gt)))
